...
public class StressMetrics
{
    ...
    private void update() throws InterruptedException
    {
        ...
        if (current.partitionCount != 0)
        {
            if (result.intervals.intervals().size() > 1)
            {
                ...
                // TODO: split hdr-histogram logs by op
                Collection<TimingInterval> intervals = result.intervals.intervals().values();
                TimingInterval merged = TimingInterval.merge(intervals, 0, 0);
                if (hlogWriter != null) {
                    Histogram hlogHistogram = merged.getExpectedTimesHistogram();
                    if (hlogHistogram.getTotalCount() > 0) {
                        hlogWriter.outputIntervalHistogram(hlogHistogram);
                    }
                }
                if (uhlogWriter != null) {
                    Histogram uhlogHistogram = merged.getActualTimesHistogram();
    ...
    public void summarise()
    {
        ...
        output.println(String.format("latency mean              : %.1f (%.1f) %s", history.meanLatency(), history.actualTimesMeanLatency(), opHistory.meanLatencies()));
        output.println(String.format("latency median            : %.1f (%.1f) %s", history.medianLatency(), history.actualTimesMedianLatency(), opHistory.medianLatencies()));
        output.println(String.format("latency 95th percentile   : %.1f (%.1f) %s", history.rankLatency(.95f), history.actualTimesRankLatency(.95f), opHistory.rankLatencies(0.95f)));
        output.println(String.format("latency 99th percentile   : %.1f (%.1f) %s", history.rankLatency(0.99f), history.actualTimesRankLatency(0.99f), opHistory.rankLatencies(0.99f)));
        output.println(String.format("latency 99.9th percentile : %.1f (%.1f) %s", history.rankLatency(0.999f), history.actualTimesRankLatency(0.999f), opHistory.rankLatencies(0.999f)));
        output.println(String.format("latency max               : %.1f (%.1f) %s", history.maxLatency(), history.actualTimesMaxLatency(), opHistory.maxLatencies()));
